...
14. **PHI/MRL (v1.4):** для пестицидов/ретардантов (class_family: fungicides, insecticides, synthetic_growth_regulators с efficacy HIGH) PHI/MRL — **блокер «практического» статуса**: если БД (EU Pesticides Database, Codex, OpenFoodTox, GB MRL) не опрошены — в карточке секция «📅 PHI и MRL» честно пишет «данные не получены» + задача PHI_MRL в task_queue; статус валидации при этом не выше `partial`.
