...
## Контракт отчёта сабагента поиска (JSON v1.2)
...
5. «Нет данных» ≠ «плохо искал»: <5 результатов по культуре → fallback-цепочка: (a) запрос без культуры + проверка упоминания культуры в тексте; (b) синонимы культуры. Всё ещё 0 → `no_data` + `search_stats`.
...
- **L1 Автопроверка:** JSON по схеме; каждый PMID через esummary (существует, заголовок матчит); DOI через Crossref.
...
Цикл на вещество: поиск по 3 культурам (сабагент) → JSON-отчёт → L1/L2 проверка → карточка + crop_evidence → corrected_dosages → task_queue.md + validation.md + log.md.
