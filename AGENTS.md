...
16. **Родственные культуры (v1.5, по итогам аудита 2026-08-06 part 5):** «фокус валидации — томат/огурец/клубника» означает **приоритет полной валидации**, а НЕ запрет на данные других культур. Данные по культурам из того же ботанического семейства (Solanaceae для томата: перец, баклажан; Cucurbitaceae для огурца: дыня, тыква, кабачок; Rosaceae для клубники: малина, ежевика) **НЕ отбрасываются**: сохраняются в `related_evidence` целевой культуры с пометкой «экстраполяция (семейство)» и указанием источника (DOI/OpenAlex ID). Они **не учитываются** при evidence_level и статусе культуры (status `no_data` сохраняется, если нет прямых работ), но сохраняются для будущего использования. Пример: данные по дыне Cucumis melo → `related_evidence` огурца.
