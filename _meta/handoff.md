...
| `_scripts/extract_report.py` | Извлечение JSON из ответа сабагента (raw_decode) |
...
**Пилот завершён и L1-проверен.** Конвейер «сабагент → JSON → оркестратор → карточка» работает. Контракт обновлён до **v1.3** (правки внесены в `AGENTS.md` и `l1_check.py`).
...
1. **Сабагенты не пишут файлы** → оркестратор извлекает JSON из ответа (`_scripts/extract_report.py`) и сохраняет артефакт в `raw/evidence/{A-Z}/<код>/`.
...
3. **Type-check в L1**: `conflicts` и `contraindications` — только массивы объектов; строки вместо dict, null-заглушки и placeholders **запрещены** (L1 падает).
4. **Europe PMC fallback**: сабагент помечает `searches.failed`, а **оркестратор сам выполняет Europe PMC-запрос** в своей среде и дополняет артефакт.
5. Оценка времени: ~5 мин/вещество на поиск (не 15–45 мин).
...
- [ ] **Europe PMC-повторные запросы** для GA3, Triacontanol, Chitosan (у них `searches.failed` с europepmc) — выполнить оркестратором и дополнять карточки препринтами (SRC:PPR).
- [ ] Проверить, что `l1_check.py` валит старые артефакты v1.2 (contract_version) — **не перезаписывать** их, они иммутабельны; только новые отчёты пишутся как v1.3.
...
4. При `searches.failed` europepmc → оркестратор выполняет Europe PMC-запрос и дополняет артефакт.
