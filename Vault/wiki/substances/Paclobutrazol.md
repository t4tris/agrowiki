...
## 📅 PHI и MRL
**Данные не получены (блокер для практики).** EU Pesticides Database и Codex — JS-приложения без
машиночитаемого API; GB MRL Register — тоже JS. PPDB (Univ. Hertfordshire) подтверждает наличие
MRL-раздела (ссылка на EU MRL database) и характеризует PBZ как вещество с регуляторными
исследованиями в ЕС и США (умеренная острая токсичность, см. Wikipedia). **Числовые MRL/PHI не
извлечены — не выдумывать.** Требуется ручной сбор: браузер → EU Pesticides Database / Codex →
Web Clipper → `raw/sources/Paclobutrazol/`. Задача в task_queue (блокер практического использования).
...
4. **PHI/MRL отсутствуют** — регуляторные БД не опрошены (высокий приоритет для практики, см. 📅 PHI и MRL)
